...
import construction from "../assets/buildings/image.png";
...
export const TodoApp = () => {
  ...
  return (
    <div className="flex justify-center flex-wrap items bg-gradient-secondary px-4 py-8 mt-11">
      
      {showAlert  && (
        <Alert className="animate-bounce right-0 bottom-9 flex flex-col w-[30%] dark:text-oxford_blue-400 z-[9999] dark:bg-red-400 absolute mt-4">
          <AlertTitle>Completely Your Tasks</AlertTitle>
          <AlertDescription>
            Complete some tasks to collect your reward!
          </AlertDescription>
        </Alert>
      )}
      {flag && showReward &&
      ...
      <div className=" absolute top-16 text-center mb-8">
        <h1 className="  text-muted-foreground font-semibold dark:text-mikado_yellow-400">
          Keep your tasks tidy and your city thriving
        </h1>
      </div>
      ...
      <div className="flex flex-wrap justify-center gap-60 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 max-w-2xl">
              ...
              <CardContent className="p-6">
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      onClick={addTask}
                      className="dark:bg-yale_blue-500 hover:opacity-90 border-0 shadow-elegant"
                    >
              ...
              {tasks.length === 0 ? (
                <Card className="border-0 bg-card/30 backdrop-blur-sm">
                  <CardContent className="p-8 text-center">
                    <p className="text-muted-foreground font-semibold dark:text-mikado_yellow-400">
                ...
                tasks.map((task) => (
                  <Card
                    key={task.id}
                    className="border-0 -z-50 bg-card/50 backdrop-blur-sm shadow-card dark:bg-oxford_blue-600 hover:shadow-elegant transition-all duration-300 animate-fade-in"
                  >
                    <CardContent className="p-4">
                      <div className="flex items-center gap-3">
                        <Checkbox
                          ...
                          className={`${task.completed? 'dark:brightness-110' : ""} mt-1 dark:bg-gold-600  brightness-50`}
                        />
                          ...
                          <p
                            className={`${
                              task.completed
                                ...
                                : "text-foreground"
                            } font-semibold  transition-all duration-200 dark:text-mikado_yellow-400`}
                          >
                            {task.text}
                          </p>
                        ...
                        <Button
                          ...
                          className="text-muted-foreground hover:invert transition-colors"
                        >
            ...
            {/* Stats */}
            {tasks.length > 0 && (
              <Card className="mt-8 border-0 dark:text-white bg-black bg-card/30 backdrop-blur-sm">
                <CardContent className="p-4">
          ...
          <CardHeader>
            {count == 0 && (
              <div className="text-muted-foreground font-semibold dark:text-mikado_yellow-400">
                Every task is a step toward something bigger.
              </div>
            )}
            {count == 1 && (
              <div className="text-muted-foreground font-semibold dark:text-mikado_yellow-400">
                You did it—another task down, and your city’s rising!
              </div>
            )}
            {count == 2 && (
              <div className="text-muted-foreground font-semibold dark:text-mikado_yellow-400">
                Nice work! That building’s standing tall because of you.
              </div>
            )}
            {count == 3 && (
              <div className="text-muted-foreground font-semibold dark:text-mikado_yellow-400">
                Your focus just laid the foundation for something great.
              </div>
            )}
            {count == 4 && (
              <div className="text-muted-foreground font-semibold dark:text-mikado_yellow-400">
                You’re on fire—your city’s growing with every move you make..
              </div>
            )}
            {count == 5 && (
              <div className="text-muted-foreground font-semibold dark:text-mikado_yellow-400">
                Another win, another wall raised. Keep going!
              </div>
            )}
            {count >= 6 && (
              <div className="text-muted-foreground font-semibold dark:text-mikado_yellow-400">
                That task? Handled. That skyline? Thanks to you.
              </div>
            )}
          </CardHeader>
          <CardDescription>
